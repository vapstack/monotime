...
// Gen provides a simple, lock-free, and strictly monotonic time source
// that generates nanosecond timestamps.
type Gen struct {
	last atomic.Int64
}

// NewGen creates a new monotonic timestamp generator.
...
func (u *UUID) UnmarshalBinary(data []byte) error {
	...
	if _, _, ok := u.parse(); !ok {
		...
		// but it's better than storing invalid monotime UUID and then passing it to NewGenUUID
		return fmt.Errorf("the provided UUID is not a monotime UUID")
	}
